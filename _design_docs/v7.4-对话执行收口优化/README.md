...
当前页面的信息密度偏低，主工作区需要在不改变各栏比例的情况下整体缩小约三分之一。本次采用根级等比缩放：

- `--ui-scale: 0.68`，即视觉尺寸保留原来的 68%，约缩小 32%。
- 根节点逻辑宽高按 `1 / 0.68` 补偿，再从左上角等比缩放，确保画面仍铺满浏览器视口。
- 字体、按钮、间距、圆角、阴影和面板同时缩放，不逐组件手工调整，避免比例漂移。
- `.app-container` 和设置页从 `100vw/100vh` 改为继承补偿后的根节点尺寸，避免缩放后右侧或底部留白。
- 分栏拖拽使用 `getBoundingClientRect().width` 计算可见宽度，保证缩放后鼠标拖动距离与分隔条移动一致。

该缩放只改变展示密度，不改变资产栏、对话栏和正文栏的百分比分配，也不改变浏览器持久化数据与导出内容。
...
- `web/src/components/BottomBar/ExecutionLogCard.module.css`
  - 日志卡固定左对齐，右侧保留缩进，内部文本统一左对齐。
- `web/src/styles/global.css`
  - 定义全局 UI 缩放比例与反向视口补偿。
- `web/src/styles/layout.css`
  - 主应用容器继承补偿后的根节点宽高，不再重复使用真实视口单位。
...
- 1280×720、1440×900 和 1920×1080 下页面缩放后均铺满视口，无右侧/底部空白。
- 缩放后分栏拖拽方向、距离与鼠标移动一致。
- 设置页与主工作区使用同一缩放比例。
...
- 执行日志改为左对齐，主页面与设置页通过根级 `0.68` 等比缩放提高整体信息密度。

本轮暂未新增独立 `turns.jsonl`。现阶段通过已有聊天记录持久化结果消息；历史执行日志的按 turn 持久化与“服务中断后机械补写结果”保留为后续增强项。
